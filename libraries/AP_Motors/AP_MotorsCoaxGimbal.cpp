...
void AP_MotorsGimbal::output_to_motors()
{
    switch (_spool_state) {
        ...
        case SpoolState::GROUND_IDLE:
            ...
            for (uint8_t i = 0; i < _num_actuators; i++) {
                rc_write_angle(AP_MOTORS_MOT_1 + i, _spin_up_ratio * _actuator_out[i] * AP_MOTORS_GIMBAL_SERVO_INPUT_RANGE);
            }
        ...
        case SpoolState::SPOOLING_DOWN:
            ...
            for (uint8_t i = 0; i < _num_actuators; i++) {
                rc_write_angle(AP_MOTORS_MOT_1 + i, _actuator_out[i] * AP_MOTORS_GIMBAL_SERVO_INPUT_RANGE);
            }
            set_actuator_with_slew(_actuator[AP_MOTORS_MOT_3], thrust_to_actuator(_thrust_yt_ccw));
            set_actuator_with_slew(_actuator[AP_MOTORS_MOT_4], thrust_to_actuator(_thrust_yt_cw));
            rc_write(AP_MOTORS_MOT_3, output_to_pwm(_actuator[AP_MOTORS_MOT_3]));
            rc_write(AP_MOTORS_MOT_4, output_to_pwm(_actuator[AP_MOTORS_MOT_4]));
            break;
    }
